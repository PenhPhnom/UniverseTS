...
import { SkillConfigTB } from '../data/excel/SkillConfig';
...
export class UnitTest {
    static async doTest() {
        ...
        Logger.log("===");
        let t1 = SingletonTest.Instance(SingletonTest);
        let t2 = SingletonTest.Instance(SingletonTest);
        ...
        try {
            Logger.log("测试红点系统 =============================");
            RedHintsMessageManager.Instance(RedHintsMessageManager).addListener(enumRedHints.chat, this, function () {
                Logger.log("red hints chat...");
            });
            RedHintsMessageManager.Instance(RedHintsMessageManager).addListener(enumRedHints.chat_family, this, function () {
                Logger.log("red hints chat_family...");
            });
            RedHintsMessageManager.Instance(RedHintsMessageManager).addListener(enumRedHints.chat_system, this, function () {
                Logger.log("red hints chat...");
            });
            RedHintsManager.Instance(RedHintsManager).setRedHintOpenOrClose(enumRedHints.chat_family, true);
            let r_chat = RedHintsManager.Instance(RedHintsManager).checkRedIsOpen(enumRedHints.chat);
            let r_chat_family = RedHintsManager.Instance(RedHintsManager).checkRedIsOpen(enumRedHints.chat_family);
            let r_chat_system = RedHintsManager.Instance(RedHintsManager).checkRedIsOpen(enumRedHints.chat_system);
        ...
        let bagC = player.addComponent(BagComponent);
        //Logger.log(bagC.name);
        let infoC = player.addComponent(PlayerInfoComponent);
